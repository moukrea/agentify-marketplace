...
You **don't**:
- Read the body of `${target_dir}/AGENTIFY.md`, `${state_root}/revisions/*.md`, `${state_root}/reviews/*.md`, or `${target_dir}/PATCH_LOG.md` during the loop body. Subagents have access to these; reading them in the parent bloats your context unbounded.
- Use `WebFetch` or `WebSearch` directly. Subagents handle research per their verification protocols.
- Modify any `${target_dir}/context/` file yourself. Subagents update them in place when needed.
...
The loop's **input** files (AGENTIFY.md, REVIEW_PROMPT.md, REVISE_AGENTIFY_PROMPT.md, PATCH_LOG.md, the `context/` bundle) live under a configurable directory called the **target dir**, denoted `target_dir` throughout this prompt.

- **Default:** `.` (the current working directory). This matches the rendered-target case: `/agentify` places AGENTIFY.md and the prompt set at the target repo's root, so bare-path references resolve correctly when `target_dir` is unset.
- **Override mechanism:** an outer caller (e.g., the marketplace's `/agt-loop` SKILL.md) MAY override the default by exporting the `TARGET_DIR` environment variable (or by setting a shell-local `target_dir` variable) before invoking this prompt. In the agentify-marketplace repo, the SKILL.md detects that AGENTIFY.md lives at `plugins/agentify/` (plugin source) and exports `TARGET_DIR=plugins/agentify` so the loop iterates on the plugin source files in place.
- **Honoring the override in bash blocks:** every bash code block below uses the parameter-expansion form `"${target_dir:-.}/AGENTIFY.md"` (and analogous forms for other input paths: `"${target_dir:-.}/PATCH_LOG.md"`, `"${target_dir:-.}/context/<file>"`, etc.). When the variable is unset, the default `.` applies and all references collapse to their bare form.
- **Subagent path substitution:** the REVISE / REVIEW / SEED subagent prompt templates in §C2, §C3, §F substitute concrete resolved paths (`<agentify_path>`, `<patch_log_path>`, `<context_dir>`) into the Inputs section before sending, so the subagent never has to resolve `target_dir` itself.
- **Narrative shorthand:** narrative-text references throughout this prompt use the shorthand `${target_dir}/AGENTIFY.md`, `${target_dir}/context/...`, etc. They mean exactly the same thing as the bash-block `"${target_dir:-.}/..."` forms — read them with the default applied unless an outer caller is in play.

## A. Orient (preamble — runs once at loop start)

Before iterating, do these checks. Print results inline.

1. Confirm working directory. The loop iterates on files at `${target_dir:-.}` (see §"Target dir parameter"). In **target mode** (rendered scaffold via `/agentify`), `target_dir=.` and the cwd is the target repo root — AGENTIFY.md and the prompt set live right there. In **marketplace mode** (the plugin source repo), the `/agt-loop` SKILL.md exports `TARGET_DIR=plugins/agentify` and the cwd is the marketplace repo root — AGENTIFY.md lives under `plugins/agentify/`. Do NOT `cd` into `${target_dir}`; the parent stays at the original cwd so that `${state_root}` (which is interpreted relative to the original cwd) resolves correctly.
...
3. Verify required files and directories exist:
   - Files: `${target_dir:-.}/AGENTIFY.md`, `${target_dir:-.}/REVIEW_PROMPT.md`, `${target_dir:-.}/REVISE_AGENTIFY_PROMPT.md`, `${target_dir:-.}/PATCH_LOG.md`, `${target_dir:-.}/context/claude-code-mechanics.md`, `${target_dir:-.}/context/known-bugs.md`, `${target_dir:-.}/context/external-research.md`, `${target_dir:-.}/context/verification-cookbook.md`
...
4. **Dirty-tree gate.** Run:
   ```
   git status --porcelain -- "${target_dir:-.}/AGENTIFY.md" "${target_dir:-.}/PATCH_LOG.md" "${target_dir:-.}/context/" "${state_root:-.agents-work}/revisions/" "${state_root:-.agents-work}/reviews/" 2>/dev/null || true
   ```
   If the output is non-empty AND `${state_root}/loop-state.json`'s `agentify_md_sha` is not null AND it does not match `sha256sum "${target_dir:-.}/AGENTIFY.md" | cut -d' ' -f1`, refuse to start. Print:
   ```
   ...
   ${state_root}/loop-state.json.agentify_md_sha = <state>; current ${target_dir}/AGENTIFY.md sha = <disk>.
   ```
...
1. Read `${state_root}/loop-state.json`. If missing, ensure the state-root directory exists (`mkdir -p "${state_root:-.agents-work}/revisions" "${state_root:-.agents-work}/reviews"`) and initialize with the schema:
   ...
   ````
   {
     ...
     "target_dir": "<resolved target_dir, e.g. \".\" or \"plugins/agentify\">",
     "last_verdict": null,
   ...
   When the `/agt-loop` SKILL.md initialised state on entry, the `target_dir` field is already populated; treat it as authoritative for the remainder of the session (so a subsequent compaction can resume without re-detecting). If the field is absent in an older state file, derive it the same way the SKILL.md does (`./AGENTIFY.md` → `.`, else `plugins/agentify/AGENTIFY.md` → `plugins/agentify`) and patch the state file in place.
...
3. Reconcile state vs disk and decide the starting mode for this run:
   ...
   - **`${state_root}/revisions/NN` exists but `${state_root}/reviews/NN` does not** → prior REVIEW was interrupted. Start with REVIEW on current `${target_dir}/AGENTIFY.md`; skip the next REVISE.
...
4. **Empty-bundle check.** The three staleness-tracked files (`claude-code-mechanics.md`, `known-bugs.md`, `external-research.md`) signal "populated" by carrying at least one `**Last verified:**` line per entry. If none of the three has any such line, the bundle is unseeded — trigger AUTO-SEED before iteration 1 (see §F). `verification-cookbook.md` is the static-reference file (no staleness) and does not factor into this check. Quick check:
   ...
   populated=$(grep -c '^\*\*Last verified:\*\*' \
       "${target_dir:-.}/context/claude-code-mechanics.md" \
       "${target_dir:-.}/context/known-bugs.md" \
       "${target_dir:-.}/context/external-research.md" \
...
revisions_dir="${state_root:-.agents-work}/revisions"
agentify_path="${target_dir:-.}/AGENTIFY.md"
patch_log_path="${target_dir:-.}/PATCH_LOG.md"
context_dir="${target_dir:-.}/context"
revise_prompt_path="${target_dir:-.}/REVISE_AGENTIFY_PROMPT.md"
```

Spawn REVISE subagent via the Agent tool, `subagent_type: general-purpose`, `description: "REVISE iteration NN"`. Substitute the `NN`, `ts`, `latest_review_path`, `parked_csv`, `revisions_dir`, `agentify_path`, `patch_log_path`, `context_dir`, and `revise_prompt_path` values into the prompt template below before sending (so the subagent sees concrete paths, e.g. `AGENTIFY.md` in target mode or `plugins/agentify/AGENTIFY.md` in marketplace mode):

````
You are the REVISE subagent for AGENTIFY iteration NN.

Read <revise_prompt_path> in full. It tells you how to apply the latest
review and produce the next AGENTIFY iteration. It also contains the
verification protocol you must use against the cached <context_dir>/ bundle.
The bare-path references in that prompt's body (e.g. `AGENTIFY.md`,
`context/...`, `PATCH_LOG.md`) are templates — every concrete path you
need is in the Inputs section below; use these and ignore the bare forms.

Inputs:
- Current AGENTIFY: <agentify_path>
- Latest review to apply: <latest_review_path>
- Cached context bundle: <context_dir>/claude-code-mechanics.md, <context_dir>/known-bugs.md,
  <context_dir>/external-research.md, <context_dir>/verification-cookbook.md
- PATCH_LOG to update: <patch_log_path>
...
Output, strictly in this order (per <revise_prompt_path> §Output order):
1. Write <revisions_dir>/NN-<ts>.md with your full Part 2 patch log.
2. Overwrite <agentify_path> with your full Part 1 (revised prompt) in a
   single Write call.
3. Prepend a one-paragraph summary entry to <patch_log_path> (top of file).
4. Compute sha256 of the new <agentify_path>: `sha256sum "<agentify_path>" | cut -d' ' -f1`.
5. End your reply with a single fenced JSON code block matching this contract
   ...
   your reply). Cite context entries with the relative `context/<file>#<anchor>`
   form (NOT the resolved `<context_dir>/...` path) so citations remain
   portable between target and marketplace modes:
...
- Do not attempt AskUserQuestion. Decision points go in the patch log per the
  protocol.
- Do not modify any file other than <agentify_path>, <patch_log_path>, the
  new <revisions_dir>/NN-<ts>.md, and any <context_dir>/*.md entries you
  refresh.
...
agentify_path="${target_dir:-.}/AGENTIFY.md"
context_dir="${target_dir:-.}/context"
review_prompt_path="${target_dir:-.}/REVIEW_PROMPT.md"
```

Spawn REVIEW subagent via the Agent tool, `subagent_type: general-purpose`, `description: "REVIEW iteration NN"`. Substitute the resolved values (`reviews_dir`, `agentify_path`, `context_dir`, `review_prompt_path`, `NN`, `ts`, `prev_review_path`, `parked_csv`) into the prompt template below before sending (so the subagent sees concrete paths, e.g. `AGENTIFY.md` in target mode or `plugins/agentify/AGENTIFY.md` in marketplace mode):

````
You are the REVIEW subagent for AGENTIFY iteration NN.

Read <review_prompt_path> in full. It tells you how to review AGENTIFY and
the verification protocol you must use against the cached <context_dir>/
bundle. The bare-path references in that prompt's body (e.g. `AGENTIFY.md`,
`context/...`) are templates — every concrete path you need is in the
Inputs section below; use these and ignore the bare forms.

Inputs:
- Target to review: <agentify_path>
...
2. End your reply with a single fenced JSON code block matching this contract
   ...
   Cite context entries with the relative `context/<file>#<anchor>` form
   (NOT the resolved `<context_dir>/...` path) so citations remain portable
   between target and marketplace modes:
...
`caused_by_prior_revise` is the count of NEW findings (any severity) you flagged with `caused_by_prior_revise: true` per the Prior-revision cross-check in <review_prompt_path>. Zero is fine; non-zero is an actionable signal — two consecutive iterations with non-zero values trigger the loop's REGRESSION exit (§C7).

Constraints:
- Do not attempt AskUserQuestion.
- Do not modify any file other than <reviews_dir>/NN-<ts>.md and any <context_dir>/*.md
...
Overwrite (do not append) `${state_root}/session-summary.md` so the parent can resume cold if the session is compacted mid-loop. Before writing, **interpolate `${state_root}` and `${target_dir}` to their concrete values** so the on-disk file records resolvable paths (consistent with how §C2/§C3 substitute `<revisions_dir>` / `<reviews_dir>` / `<agentify_path>` into subagent prompts). The `<revision_path>` / `<review_path>` placeholders are filled from this iteration's REVISE/REVIEW JSON output. Use this template:
...
**AGENTIFY source:** <agentify_path> (sha256: <sha>)
...
Target dir: <target_dir>
Live AGENTIFY.md: ${target_dir}/AGENTIFY.md (sha: <sha>)
...
context/ entries created or refreshed during this run (cite as relative
`context/<file>#<anchor>` for portability; the concrete files live under
${target_dir}/context/):
  - context/<file>#<anchor>
  - ...
...
Suggested next step:
  ...
  PARKED           → review parked items; either fix upstream or accept,
                     then resume the loop with updated ${target_dir}/context/.
  REGRESSION       → the reviser claimed Applied for code-bearing fixes that
                     ...
                     ${target_dir}/REVISE_AGENTIFY_PROMPT.md if a pattern
                     emerges. Do NOT resume the loop without addressing this —
                     more iterations will compound the regressions.
  STALLED          → inspect last review; reviewer/reviser may be disagreeing
                     on a ${target_dir}/context/ entry. Verify the entry
                     against its source, refresh in place, then resume.
...
Triggered from §B step 4 when `${target_dir}/context/*.md` files are header-only. Spawn a SEED subagent via the Agent tool, `subagent_type: general-purpose`, `description: "SEED context/ bundle"`. Resolve the input paths first:
...
Then substitute the resolved values into the prompt template below:

````
You are the SEED subagent for the agentify project. The <context_dir>/ bundle
is empty (header-only files). Populate it.

Inputs (concrete paths for this run):
- Context bundle directory: <context_dir>
- Review prompt: <review_prompt_path>
- Revise prompt: <revise_prompt_path>
- AGENTIFY source: <agentify_path>
- Patch log: <patch_log_path>

Sources to consult, in order:
1. <review_prompt_path> — the seed-list bullets at the top of each
   <context_dir>/*.md file mirror the topics this prompt enumerates.
   Cross-reference with the verification protocol for the bundle structure.
2. <revise_prompt_path> — its verification protocol enumerates the
   verification patterns to seed in <context_dir>/verification-cookbook.md.
3. <agentify_path> — Acknowledgements section (near the bottom, before
   "End of prompt.") — primary references list for
   <context_dir>/external-research.md.
4. <patch_log_path> — issue numbers and version-stamped decisions to seed
   <context_dir>/known-bugs.md.
...
Output:
- Overwrite all four <context_dir>/*.md files. KEEP the existing header
  preamble (refresh-policy, anchor-stability, spot-check rules); replace only
  the seed-list portion below the header with actual anchored subsections.
- End your reply with a single fenced JSON block. Cite the files with the
  relative `context/<file>` form (NOT the resolved `<context_dir>/...` path)
  so the JSON stays portable between target and marketplace modes:
...
Constraints:
- Do not modify <agentify_path>, <review_prompt_path>, <revise_prompt_path>,
  or <patch_log_path>.
- Do not attempt AskUserQuestion.
...
- After a `DONE` exit, the human is invited to consolidate the iteration's revisions into a single `## Patch log — vN to vN+1` section in `${target_dir}/PATCH_LOG.md` (above any prior version section) and bump the version in `${target_dir}/AGENTIFY.md`'s H1.
