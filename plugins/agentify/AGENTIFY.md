...
The §6 loop ships a `.claude/settings.loop.json` overlay activated when `/{__AGT_SKILL_PREFIX__}-loop start` runs. Drop-in in §12.14. The overlay adds `sandbox` (network allowlist for Bash) **and** `permissions` rules for WebFetch (default-deny + explicit allow) — the sandbox doesn't govern WebFetch on its own. The overlay's SessionStart block writes sentinel files synchronously to BOTH `$(git rev-parse --git-common-dir)/.loop-overlay-active` (worktree-shared; see §6.5) AND `${CLAUDE_PROJECT_DIR}/.agents-work/.loop-overlay-active` (project-dir fallback for non-git directories) so §5.5 #18 can detect when the overlay is loaded (either present means active); no environment variables are written (a previous draft set `EG_LOOP_OVERLAY=1` via `$CLAUDE_ENV_FILE`, which had no consumer and risked re-introducing the same-batch race that the sentinel design closed).
...
18. **SessionStart `startup` — loop overlay check** (`loop-overlay-check.sh`, §12.22). When `.agents-work/loop-state.json` is present but BOTH overlay sentinels are missing (`$(git rev-parse --git-common-dir)/.loop-overlay-active` AND `${CLAUDE_PROJECT_DIR}/.agents-work/.loop-overlay-active`), prints a stderr warning that the loop sandbox + WebFetch deny rules are not in effect and the session is running with the base permissions. With either sentinel present, the warning is suppressed. Detection uses sentinel files rather than `EG_LOOP_OVERLAY` env var because `CLAUDE_ENV_FILE` writes from a sibling SessionStart hook are not visible inside the same SessionStart batch (`context/claude-code-mechanics.md#hooks`); the env-file sources before *subsequent* Bash, not before sibling hooks. Honors `EG_LOOP_CHECK_QUIET=1` for known-good sessions.
...
30. **Loop overlay check**: with `loop-state.json` present and BOTH sentinel locations absent (`${CLAUDE_PROJECT_DIR}/.agents-work/.loop-overlay-active` AND `$(git rev-parse --git-common-dir)/.loop-overlay-active`), `loop-overlay-check.sh` prints a stderr warning at SessionStart. With either sentinel present, the warning is suppressed (the §12.22 check short-circuits on the first present sentinel — OR-semantics, not AND). With `EG_LOOP_CHECK_QUIET=1`, suppressed regardless. Additionally: `git push --force-with-lease` is blocked by `guard-bash.sh`. Additionally: after `/{__AGT_SKILL_PREFIX__}-loop reset`, `state.json.current_work.blocked_on == null` and `state.json.current_phase == "idle"`.
...
- **Cross-section prose drift inside AGENTIFY.md itself.** When iter-2 fixed the Stop allow-shape drift in §12.6 and §10, the same wording in §5.5 hook description and §Acknowledgements was left at the deprecated form, producing an internally self-contradictory prompt where the hook layering description authoritatively documented the deprecated shape while §10 anti-patterns and §12.6 implementation used the documented shape — a future reviser following §5.5's framing would re-introduce the original bug citing AGENTIFY.md as source-of-truth. Operational rule: when adding a new §10 entry that supersedes or contradicts a prior cross-section claim, edit every cross-section instance of the prior claim in the same patch (every §5 hook description, every §8 verification check, every §10 anti-pattern, every §12 implementation, every §Acknowledgements citation) rather than adding a corrective in only one place. Drive the enumeration from a phrase-based site-grep against the prior wording, not from the finding's named sites — scoping edits to only the named sites leaves the un-enumerated drift the reviewer's cross-check flags as `caused_by_prior_revise`. Closes review 02 M-doc-drift-1 / M-doc-drift-2 / Mo-ack-drift cluster, review 02 P1, and review 06 Mo1+Mo2 (iter-06 P1 enumerated §5.1/§5.9/§6.4 but missed §5.5 #18 + §8 #30 in the same drift class — the §8 verification-check site was off the original enumeration, which this iteration adds explicitly).
...
```bash
#!/usr/bin/env bash
# .claude/hooks/loop-overlay-check.sh — warn when loop-state present but BOTH overlay sentinels missing (git-common-dir AND project-dir).
...
cat >&2 <<'EOF'
[loop-overlay-check] loop-state.json is present but BOTH loop overlay
sentinels are missing ($(git rev-parse --git-common-dir)/.loop-overlay-active
AND ${CLAUDE_PROJECT_DIR}/.agents-work/.loop-overlay-active). The sandbox
network allowlist and WebFetch deny rules are NOT in effect for this session.
