...
1. **Site-grep.** Grep `${target_dir:-.}/AGENTIFY.md` for every other mention of the value being changed. Pick the pattern that matches the change class:
   ...
   - Cross-section prose drift (M-doc-drift class — a behavior described at §5 hook descriptions, §8 verification checks, §10 anti-patterns, §12 implementations, §Acknowledgements citations): grep for the behavior's distinctive token (variable name, path, signature) rather than the finding's named-site list. Example: `grep -nE '\.loop-overlay-active' "${target_dir:-.}/AGENTIFY.md"` enumerates every sentinel-mentioning site for the dual-sentinel fix; walk every hit, do not trust the finding's named-site enumeration. Review 06 Mo1+Mo2 walked into this: iter-06 P1's named-site list of §5.1/§5.9/§6.4 missed §5.5 #18 hook description and §8 #30 verification check, both inside the drift class the patch was scoped to close.
...
5. **Post-write forward-pointer resolvability — distinct from same-value drift.** Steps 1–4 catch "value X appears in N places, update them all." They do NOT catch a *forward pointer* introduced in this iteration — a `see §N.M`, an anchor jump, or a `line NN` prose citation whose target may not exist (or may sit at a different line) by the time the final Write lands. For every newly-introduced cross-reference, after the final Write to `${target_dir:-.}/AGENTIFY.md`, run a resolvability check matched to the reference class:
   - Section pointer (`see §N.M`, `cross-reference to §N`): `grep -nE '^#+ §N\.M' "${target_dir:-.}/AGENTIFY.md"` returns exactly one match — the heading. Cite the matched line in the Verification block.
   - Anchor jump (`#kebab-anchor-id`): `grep -nF '{#anchor-id}' "${target_dir:-.}/AGENTIFY.md"` returns exactly one match.
   - Line-number citation in prose (`at line 2190`, `lines 2196–2200`): `sed -n 'NNp' "${target_dir:-.}/AGENTIFY.md"` — the line content matches the citing prose's description (or falls within the stated window). When prose cites a range, spot-check both ends.
   - Pointer phrase without an anchor (`see §5.5 #12 wrapper note`): grep for the destination subsection's distinctive phrase to confirm the description still applies post-edit.

   Forward references without a post-write resolvability check are NOT Applied; downgrade to **Partially applied** with the failing grep/sed output, or to **Not applied**. Both `caused_by_prior_revise` findings in iter-04 and iter-05 of this loop's history were forward-pointer drift that the same-value site-grep (steps 1–4) did not catch — iter-03 added a `see §10 "CPD scoping invariants"` pointer to an anchor that didn't yet exist; iter-04 created the §10 entry but its prose cited the §12.4 forwarding pointer at "line 2190" when the actual location was "lines 2196–2200" (iter-05 caught this). Treat forward-pointer resolvability as a separate check class; the post-write `sed`/`grep` proof goes in the Verification block alongside any site-grep counts.

Cross-section findings without a site-grep Verification block (steps 1–4) AND, where forward pointers were introduced, without a resolvability Verification block (step 5), are NOT Applied. Use **Partially applied** with the grep/sed output proving the remaining stale sites or unresolved pointers, or **Not applied** with a justification. The reviewer will re-execute the same site-grep AND resolvability check on the next iteration and flag any remaining staleness as `caused_by_prior_revise: true`.
...
9. Every Applied finding that changes a value, name, count, or pointer carries a site-grep `**Verification:**` sub-bullet per the Cross-section consistency gate steps 1–4, and the post-edit count reflects every site updated to the new value (or explicitly preserved with a one-line reason). The H1 version bump alone has at least three known sites — the H1 itself, the §12.16 / §10 uninstall regex enumeration of supported versions, and any §7.x backfill-format references; all three must move in lockstep. Stale same-value sites are one dominant `caused_by_prior_revise` class observed in this loop's history.
10. Every Applied finding that introduces a NEW forward reference (a `§N.M` section pointer, a `{#anchor}` jump, a `line NN` or `lines NN–MM` prose citation, or a `see §X` pointer phrase without an anchor) carries a post-write resolvability `**Verification:**` sub-bullet per the Cross-section consistency gate step 5 — a `grep -nE` for the section heading, a `grep -nF` for the anchor marker, or a `sed -n 'NNp'` for the cited line(s). Forward-pointer drift was the entire `caused_by_prior_revise` content in iter-04 and iter-05; treat it as a separate check class from same-value drift, because step 1's site-grep does not catch it.
